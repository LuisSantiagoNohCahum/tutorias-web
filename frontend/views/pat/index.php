    ...
    <?php } else {?>
        <?php if (count($dataProviderSemanas->getModels())>0) {?>
            <?= GridView::widget([
                ...
                'condensed'=>true,
                'options' => [
                    'class'=>'table table-sm'//table-light table-bordered
                ],
                'rowOptions' => [
                    'class'=>'border-dark'
                ],
                'columns' => [
                    ...
                    [
                        ...
                        'urlCreator' => function ($action, $model, $key, $index, $column) {
                            ...
                            if ($action=='view') return Url::toRoute(['/semana/'.$action, 'id' => $model->id]);

                            return Url::toRoute(['/semana/'.$action, 'id' => $model->id]);
                        },
                        'viewOptions'=>[
                            'label'=> '<i class="fas fa-eye"></i>',
                            'class'=>'btn btn-outline-dark pb-1 pt-1 pl-2 pr-2',
                        ],
                        /* 'buttonOptions'=>[
                            'view'=> [
